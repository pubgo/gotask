...
func _NewGroupTask(max int, maxDur time.Duration) *_GroupTask {
	_t := &_GroupTask{
		max:     max,
		maxDur:  maxDur,
...
func (t *_GroupTask) Size() int {
	return len(t.taskL)
}

// ATLen current active task size
func (t *_GroupTask) CurSize() int {
	t.mux.Lock()
	defer t.mux.Unlock()
...
func (t *_GroupTask) Wait() {
	for len(t.taskL) > 0 {
		time.Sleep(time.Second)
	}
}

func (t *_GroupTask) Stat() Stat {
	return Stat{
		QL:     t.Size(),
...
func (t *_GroupTask) Do(name TaskFn, args ...interface{}) {
	t.mux.Lock()
	defer t.mux.Unlock()
...
func (t *_GroupTask) _taskHandle(fn func(...interface{}) (err error)) {
	_t := time.Now()
